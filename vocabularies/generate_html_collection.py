...
from settings import ONTOLOGIES, SPARQL_HOST_NAME, SPARQL_DATASET, \
    HTML_DIRECTORY, GLOSSARY, ONTOLOGY_MAP, COLLECTION_MAP, DISPLAY_NAME_MAP, \
...
def write_head(ontology_name, collection_uri, found_properties):
    ...
    FILE.write('<title>The %s %s Collection</title>\n' %
    ...
    FILE.write('<h1>The %s %s Collection</h1>\n' %
               (DISPLAY_NAME_MAP[ontology_name], title))
...
def do_stuff(ontology_name, collection_uri):
    ...
    HELPER.write_entities(ontology_name, members, 'members',
                          'Members', nerc_members)
    FILE.write('</div>\n')
...
def generate():
    for ontology in ONTOLOGIES:
        ...
        for collection in collections:
            ...
            HELPER.BASE_URI = (
                '%s%s' % (COLLECTION_MAP[ontology], collection_name))
            FILE_BASE_URI = ('http://%s/collection/%s/%s-content/%s' %
                             (SPARQL_HOST_NAME, ontology, ontology,
                              collection_name))
            file_name = os.path.join(HTML_DIRECTORY, 'collection', ontology,
                                     ontology + '-content',
