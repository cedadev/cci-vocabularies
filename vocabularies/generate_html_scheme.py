...
from settings import ONTOLOGIES, SPARQL_HOST_NAME, SPARQL_DATASET, \
    HTML_DIRECTORY, CITO, CMIP_SCHEME, GCOS_SCHEME, \
    GRIB_SCHEME, CCI, CMIP, GCOS, GRIB, GLOSSARY, \
    SCHEME_MAP, ONTOLOGY_MAP, DISPLAY_NAME_MAP, \
    SPARQL_GRAPH, SPARQL_QUERY
...
def write_head(ontology_name, found_classes, found_properties):
    ...
    FILE.write('<title>The %s %s Concept Scheme</title>\n' %
    ...
    FILE.write('<h1>The %s %s Concept Scheme</h1>\n' %
               (DISPLAY_NAME_MAP[ontology_name], title))
    ...
    FILE.write('<dd><a href="%s">SPARQL endpoint</a>, dataset:%s, graph:%s/%s</dd>\n'
...
def do_stuff(ontology_name):
    ...
    HELPER.write_entities(ontology_name, concepts_schemes, 'conceptschemes',
                          'Concept Schemes')
    FILE.write('</div>\n')
...
def generate():
    for ontology in ONTOLOGIES:
        global PREFIX, SCHEME_BASE_URI, TOP_ONTOLOGY_BASE_URI, FILE_BASE_URI, \
            FILE, HELPER
        HELPER = Helper()
        HELPER.PREFIX = ('PREFIX %s:   <%s> %s' %
                         (ontology, ONTOLOGY_MAP[ontology], PREFIX))
        HELPER.TYPE = 'scheme'
